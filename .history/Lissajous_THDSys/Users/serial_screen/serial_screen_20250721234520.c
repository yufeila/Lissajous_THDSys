...
{
    static const float kx = (LCD_X_MAX - LCD_X_MIN) / 4095.0f;
    static const float ky = (LCD_Y_MAX - LCD_Y_MIN) / 4095.0f;
    ...
    while (tail != head)
    {
        ...
        for (int i = 0; i < N_POINTS; ++i)
        {
            ...
            dot_xy[i*2]     = LCD_X_MIN + (uint16_t)(x_adc * kx);
            dot_xy[i*2 + 1] = LCD_Y_MIN + (uint16_t)(y_adc * ky);
        }

        GUI_ConDots(0, dot_xy, N_POINTS);   // 一条指令批量画 200 点
    }
}
