...
void ProcessMessage( PCTRL_MSG msg, uint16 size )
{
    ...
    case NOTIFY_READ_FLASH_OK:                                                      //读取FLASH成功
        // 读取寄存器成功回调：msg->param[0..] 保存了读回的数据
        {
            uint16_t addr = PTR2U16(msg->param);       // 读请求地址
            uint16_t len  = PTR2U16(msg->param + 2);   // 读寄存器长度
            if (addr == 0x0000 && len == 1)            // duty_set 寄存器
            {
                // msg->param[4,5] 是寄存器值高/低字节
                duty_set = ((uint16_t)msg->param[4] << 8)
                        | ((uint16_t)msg->param[5]);
                // TODO: 按需处理新的 duty_set，如配置定时器占空比
                SetDuty(duty_set);
            }
        }
...
void NotifyButton(uint16 screen_id, uint16 control_id, uint8  state)
{
    ...
    else if (screen_id == PAGE_MEASURE && control_id == CTRL_BIN_MEASURE_THD)
    {
        ...
        char thd_str[8];
        snprintf(thd_str, sizeof(thd_str), "%.1f%%", measured_thd * 100.0f);
...
void NotifyText(uint16 screen_id, uint16 control_id, uint8 *str)
{
    if (screen_id == PAGE_MEASURE && control_id == CTRL_TEXT_DUTY) {
        ...
        SetTextFloat(PAGE_MEASURE, CTRL_TEXT_DUTY, duty_value, 1, 1);
    }
    // 其它文本控件逻辑可继续补充
}
