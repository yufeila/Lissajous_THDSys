...
void UpdateUI()
{
    if(current_screen_id == PAGE_MEASURE)
    {
        SetTextInt32(PAGE_MEASURE, CTRL_TEXT_DUTY, duty_set, 0, 1);
    }
    if(current_screen_id == PAGE_LISSAJOUS)
    {
        SetTextInt32(PAGE_LISSAJOUS, CTRL_TEXT_PLL_MULTIPLY, ppl_value, 0, 0);
    }
}
