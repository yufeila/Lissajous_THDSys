...
void UpdateUI(void)
{
    if (current_screen_id == PAGE_LISSAJOUS)
    {
        ...
        // 如果用户请求了相位检测，就在这里做一次
        if (phase_detect_request) 
        {
            float phase_value = 0;
            Phase_Detect(&phase_value);
            // 把结果显示到屏幕上的文本控件
            SetTextFloat(PAGE_LISSAJOUS,CTRL_TEXT_PHASE_VAL,phase_value,0,1);
            phase_detect_request = 0;
        }
...
void NotifyButton(uint16 screen_id, uint16 control_id, uint8  state)
{
    ...
    {
        // phase_detect_request = 1;
        Phase_Detect_Test(90.0f); // 调用测试函数，测试90度相位差
    }
