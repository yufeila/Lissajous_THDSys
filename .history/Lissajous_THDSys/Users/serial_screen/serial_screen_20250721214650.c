...
#define CTRL_BIN_LISSAJOUS_DISPLAY 4   // “锁相环倍频”绑定倍频值的文本控件
...
void NotifyText(uint16 screen_id, uint16 control_id, uint8 *str)
{
    ...
    {
        uint8_t ppl_value = 0.0f;
        sscanf((const char*)str, "%f", &ppl_value);
        SetPLLMultiply((uint16_t)ppl_value);
        SetTextFloat(PAGE_LISSAJOUS, CTRL_TEXT_PLL_MULTIPLY, ppl_value, 1, 1);
    }
}
