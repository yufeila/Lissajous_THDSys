...
/* 外部变量声明 */
extern volatile uint16_t adc3_buffer[N_RAW];
...
{
    printf("init_hann_fft: Starting initialization...\r\n");
    if (!win_ready) {
        printf("init_hann_fft: Initializing Hann window...\r\n");
        for (uint32_t n = 0; n < N_RAW; ++n)
            win[n] = 0.5f * (1.0f - cosf(2.0f * PI_F * n / (N_RAW-1)));
        win_ready = 1;
        printf("init_hann_fft: Hann window ready\r\n");
    }
    if (!fft_ready) {
        printf("init_hann_fft: Initializing FFT config for N_FFT=%d...\r\n", N_FFT);
        arm_rfft_fast_init_f32(&cfg, N_FFT);
        fft_ready = 1;
        printf("init_hann_fft: FFT config ready\r\n");
    }
    printf("init_hann_fft: Initialization complete\r\n");
}
 ...
 * 自动搜索基波频率（最大幅值bin），再搜索各次谐波的幅值，
 * 并计算总谐波失真（THD）等参数。
 *
 * @param[in]  adc      输入的采样信号数组，长度为N_RAW
 * @param[out] out      结果输出结构体，包含基波、各次谐波幅值和THD百分比
 */
void fft_tdh_hann(const float *adc, thd_result_t *out)
{
    init_hann_fft();  // 初始化窗口和FFT
    
    printf("fft_tdh_hann: Starting, N_FFT=%d\r\n", N_FFT);
    ...
    printf("fft_tdh_hann: Window applied, calling FFT...\r\n");

    // 2. FFT
    arm_rfft_fast_f32(&cfg, buf, buf, 0);
    
    printf("fft_tdh_hann: FFT completed\r\n");
    ...
    // 自动搜索基波bin（最大幅值bin，排除直流分量）
    float maxVal;
    uint32_t maxIndex;
    arm_max_f32(&mag[1], N_FFT/2 - 1, &maxVal, &maxIndex);  // 去掉直流分量
    uint32_t k1 = maxIndex + 1;
    float f1 = k1 * f_bin;
...
void MeasureTHD(float * thd_value)
{
    ...
    *thd_value = 5.0f;  // 返回5%作为测试值
    ...
    printf("MeasureTHD: Flag reset to NONE\r\n");

    // 清除可能的DMA标志位
    __HAL_DMA_CLEAR_FLAG(&hdma_adc3, DMA_FLAG_TCIF0_4);

    // 先启动ADC+DMA，确保准备就绪
    if (HAL_ADC_Start_DMA(&hadc3, (uint32_t*)adc3_buffer, N_RAW) != HAL_OK)
    {
        printf("MeasureTHD: ADC_Start_DMA failed!\r\n");
        Error_Handler();
    }       
    printf("MeasureTHD: ADC+DMA started\r\n");

    // 启动定时器，第一次TRGO将在完整周期后发生
    HAL_TIM_Base_Start(&htim3);
    printf("MeasureTHD: TIM3 started\r\n");
    ...
    if (timeout >= 1000)
    {
        printf("MeasureTHD: TIMEOUT! ADC3_BufferReadyFlag=%d\r\n", ADC3_BufferReadyFlag);
    ...
    // 5. 处理本轮采集的数据
    const uint16_t *src = (const uint16_t *)&adc3_buffer[0];
