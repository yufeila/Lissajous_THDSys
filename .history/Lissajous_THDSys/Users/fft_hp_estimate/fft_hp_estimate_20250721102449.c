...
void fft_tdh_hann(const float *adc, float f0_hint, thd_result_t *out)
{
    ...
    /* --------- 0. 预处理：把复频谱转幅值谱 --------- */
    static float mag[N_FFT/2];                 /* 单边幅值谱 */
    arm_cmplx_mag_f32(buf, mag, N_FFT/2);      /* buf 是 RFFT 输出 */

    /* --------- 1. 遍历 1~7 次谐波，用 DSP 库找峰 --------- */
    #define SEARCH   2                         /* 峰值搜索半宽 (±2 bin) */
    for (uint8_t n = 1; n <= HARM_MAX; n++)    /* n = 1..7 */
    {
        uint32_t k_est = k1 * n;               /* 该次谐波理论中心 bin */
        if (k_est >= N_FFT/2) break;           /* 越界保护 */

        /* 取 [start, end] 窗口，找局部最大 */
        ...
        uint32_t k_peak = start + peakIdxLocal;    /* 真正峰值 bin，可留作调试 */

        /* --------- 2. 幅值补偿 → Vrms --------- */
        /* 单边→峰值：RFFT 单边谱需 ×2；再除以样本数和 Hann 能量系数 */
        float Vpk = 2.0f * peakVal / ((float)N_RAW * HANN_CG);
        Vn[n] = Vpk * 0.70710678f;                /* 峰值转 rms */
    }
