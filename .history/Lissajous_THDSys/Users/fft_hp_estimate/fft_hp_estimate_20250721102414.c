...
void fft_tdh_hann(const float *adc, float f0_hint, thd_result_t *out)
{
    ...
    /* ---- 3. 计算每个谐波幅值 ---- */
    float fs = FS_HZ;                    /* 采样率 (宏或运行时变量)  */
    float f_bin = fs / (float)N_FFT;     /* 每个 bin 的频差         */
    uint32_t k1 = (uint32_t)roundf(f0_hint / f_bin);   /* 基波 bin */

    float Vn[HARM_MAX+1] = {0};          /* 0~7 次谐波幅值 (峰值)   */

    for (uint8_t n=1; n<=HARM_MAX; n++)  /* n = 1..7 */
    {
        uint32_t k = k1 * n;
        if (k >= N_FFT/2) break;         /* 防越界 */

        float re = buf[2*k], im = buf[2*k+1];
        /* Hann 窗幅值补偿 & 单边 → 峰值 (Vpk) */
        float Vpk = 2.0f * sqrtf(re*re + im*im) / ((float)N_RAW * HANN_CG);
        /* 峰值转 rms：Vrms = Vpk / √2 */
        Vn[n] = Vpk * 0.70710678f;
    }
