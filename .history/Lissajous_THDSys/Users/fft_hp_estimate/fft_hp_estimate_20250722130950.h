...
#define N_RAW      1024                /* 临时减小到1024测试 */
#define Z_FACTOR   1u                   /* 临时不补零，减小FFT点数 */
#define N_FFT      (N_RAW * Z_FACTOR)   /* FFT 点数 (必须2的幂)     */
#define PSC         (TIM2-> PSC)
#define ARR         (TIM2-> ARR)
#define TIMCLK      84000000.0f         /* APB × 2或实际值         */
#define FS_HZ      48000.0f           /* 采样率48kHz       */
#define HARM_MAX   7                     /* 计算到 7 次谐波 */
#define THD_SCALE  100.0f               /* 输出百分比 */
...
/* 方波失真度分析结果结构体 */
typedef struct {
    float fundamental;          /* Hz */
    float V_rms[HARM_MAX+1];    /* 0~7 次谐波的 rms 幅值 */
