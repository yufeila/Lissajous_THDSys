...
{
    static const float kx = (X_SCREEN_MAX - X_SCREEN_MIN) / 4095.0f;
    static const float ky = (Y_SCREEN_MAX - Y_SCREEN_MIN) / 4095.0f;
    ...
    while (tail != head)
    {
        ...
        for (int i = 0; i < N_POINTS; ++i)
        {
            ...
            dot_xy[i*2]     = X_SCREEN_MIN + (uint16_t)(x_adc * kx);
            dot_xy[i*2 + 1] = Y_SCREEN_MIN + (uint16_t)(y_adc * ky);
        }

        GUI_ConDots(0, dot_xy, N_POINTS);   // Ò»ÌõÖ¸ÁîÅúÁ¿»­ 200 µã
    }
}
