...
void Lissajous_Point_Config(lissajous_point_t* pt, uint16_t x_adc, uint16_t y_adc)
{
    ...
    // Ó³Éäµ½ÆÁÄ»×ø±ê
    pt->x_label = X_SCREEN_MIN + (uint16_t)((pt->x_voltage / ADC_REF_VOLTAGE) * (X_SCREEN_MAX - X_SCREEN_MIN));
    pt->y_label = Y_SCREEN_MIN + (uint16_t)((pt->y_voltage / ADC_REF_VOLTAGE) * (Y_SCREEN_MAX - Y_SCREEN_MIN));
}
