...
void AD9833_Config(float fout, uint16_t waveform, uint16_t phase)
{   
    ...
    AD9833_SetRegisterValue(ctrl | AD9833_RESET);          // 复位 + 设波形
    HAL_Delay(1);  // 确保复位稳定
    AD9833_SetRegisterValue(AD9833_REG_FREQ0 | (freq & 0x3FFF));      // LSB
    AD9833_SetRegisterValue(AD9833_REG_FREQ0 | (freq >> 14));         // MSB
    AD9833_SetRegisterValue(AD9833_REG_PHASE0 | phase_word);          // 设置相位
    HAL_Delay(1);  // 确保寄存器写入稳定
    AD9833_SetRegisterValue(ctrl);                         // 清 RESET，开始振荡
}
